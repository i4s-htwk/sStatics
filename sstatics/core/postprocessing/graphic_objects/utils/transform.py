...
class Transform:
    ...
    rotation around an origin, uniform scaling, and translation before and
    after rotation and scaling. By centralizing these operations, all graphic
    objects can consistently transform their coordinates without duplicating
    logic.
    ...
    def __init__(
        ...
        self._validate(
            origin, rotation, scaling, pre_translation, post_translation
        )
        self._origin = origin
        self._rotation = rotation
        self._scaling = scaling
        self._pre_translation = np.array(pre_translation, dtype=float)
        self._post_translation = np.array(post_translation, dtype=float)
    ...
    def apply(
        ...
        0. Translation before rotation and scaling.
        1. Rotation around the origin.
        2. Scaling relative to the origin.
        3. Translation after rotation and scaling.
        ...
        -------
        >>> t = Transform(
        >>>     origin=(1, 1), rotation=np.pi/2, scaling=2,
        ...
        x, z = self._pre_translate(x, z)
        x, z = self._rotate(x, z)
        x, z = self._scale(x, z)
        x, z = self._post_translate(x, z)
        return x, z

    def _pre_translate(self, x, z):
        """Translate points by the defined pre_translation vector.
        ...
        Notes
        -----
        The pre_translation is applied before rotation and scaling, shifting
        all coordinates by the specified offset in :py:attr:`_pre_translation`.
        """
        x_shifted = x + self._pre_translation[0]
        z_shifted = z + self._pre_translation[1]
    ...
    def _post_translate(self, x, z):
        """Translate points by the defined post_translation vector.
        ...
        Notes
        -----
        The post_translation is applied after rotation and scaling, shifting
        all coordinates by the specified offset in
        :py:attr:`_post_translation`.
        """
        x_shifted = x + self._post_translation[0]
        z_shifted = z + self._post_translation[1]
        return x_shifted, z_shifted
    ...
    def _validate(
        ...
        if (
                not isinstance(pre_translation, tuple)
                or len(pre_translation) != 2
        ):
            raise TypeError(
                f'pre_translation must be a tuple of length 2, '
                f'got {pre_translation!r}.'
            )

        if not all(isinstance(v, (int, float)) for v in pre_translation):
            raise TypeError(
                f'pre_translation must contain numbers, got '
                f'{pre_translation!r}.'
            )

        if (
                not isinstance(post_translation, tuple)
                or len(post_translation) != 2
        ):
            raise TypeError(
                f'post_translation must be a tuple of length 2, '
                f'got {post_translation!r}.'
            )

        if not all(isinstance(v, (int, float)) for v in post_translation):
            raise TypeError(
                f'post_translation must contain numbers, got '
                f'{post_translation!r}.'
            )
    ...
    def __repr__(self):
        ...
        str
            ...
            e.g.:
            ``Transform(
            origin=(0, 0), rotation=1.571, scaling=1.0, pre_translation=(1, 0),
        ...
        return (
            ...
            f'rotation={self._rotation:.3f}, '
            f'scaling={self._scaling}, '
            f'pre_translation={tuple(self._pre_translation)}, '
            f'post_translation={tuple(self._post_translation)}'
            f')'
        )
