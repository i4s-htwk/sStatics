...
class Bar:
    ...
    def modified_flexural_stiffness(self):
        ...
        if 'shear' in self.deformations:
            return self.EI * (1 + self.f0_line[0][0] / self.GA_s)
        else:
            return self.EI
    ...
    def stiffness_second_order_p_delta(self):
        """ TODO """
        c = self.f0_line[0][0] / self.length
        return np.array([
            [0, 0, 0, 0, 0, 0],
            [0, c, 0, 0, -c, 0],
            [0, 0, 0, 0, 0, 0],
    ...
    def f0(
        ...
        if order == 'first':
            ...
            if approach == 'analytic':
                f0_line = self.f0_line_analytic
            elif approach == 'taylor':
                f0_line = self.f0_line_taylor
            else:
                f0_line = self.f0_line
